...
function enviarLead() {
  ...
  const lead = {
    ...
    nome: a.nomeResp || "", whatsapp: a.whatsapp || "", email: a.email || "", instagram: a.instagram || "",
    tempo: label("tempo"), relacao: label("relacao"), travamento: label("travamento"),
    impacto: label("impacto"), custo: label("custo"), tentativas: label("tentativas"),
    objetivo: label("objetivo"), perfil: label("perfil"), frequencia: label("frequencia"),
    intencao: label("intencao"), frente: "Mentoria Mecha", origem: document.referrer || location.href,
    ...URL_UTMS,
  };
  try {
...
/* ---------- start ---------- */
(function init() {
  trackEvent("page_view", { funil: "mecha" });
