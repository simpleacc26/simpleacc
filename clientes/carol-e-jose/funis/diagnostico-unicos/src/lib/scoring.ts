...
export function aprovacao(a: Answers): Aprovacao {
  ...
  const fat = parseInt(a["faturamento"] || "0", 10); // 2..5 (1 já saiu no piso)
  const industria = a["setor"] === "industria";
  const corteIcp = industria ? 3 : 2; // índice mínimo de faixa para pleno
