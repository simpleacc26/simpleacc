...
| **`/copy-hormozi`** | Comando (copy/roteiro) | Vira um **copywriter de storytelling no modelo Hormozi**: hook de identidade, problema reformulado, custo de continuar, virada de crença, prova e CTA construído, com filtro de ICP embutido no meio do texto. Entrega **copies estáticas + roteiros de vídeo** em texto corrido. Conduz diagnóstico → geração. | Digite `/copy-hormozi` em qualquer sessão. Na pasta de um cliente, já puxa ICP, voz e ângulos já usados. |
| **`/mesa-de-guerra`** | Comando (decisão de negócio) | Monta um **conselho estratégico de 4 cabeças** (Russell Brunson, Alex Hormozi, Luiz Filho e Matuta) para decidir: oferta, preço, escala, time, operação, caixa ou crise. Cada conselheiro entrega leitura, riscos, solução proporcional, ajuste necessário e critério objetivo de sucesso; no fim sai a **síntese** com convergências, divergências e a prioridade da semana / mês / trimestre. Não responde antes do diagnóstico e não chuta o que não foi informado. | Digite `/mesa-de-guerra` e apresente a questão. Na pasta de um cliente, já usa o contexto e os aprendizados dele. |
