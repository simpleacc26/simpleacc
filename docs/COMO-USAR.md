...
| **Carrossel de Instagram** (`carrossel-instagram`) e **`/carrossel`** | Skill + comando | Transforma um **tema + a identidade da marca** em um carrossel 4:5: gera o **preview em HTML** dentro de uma moldura do Instagram (para o cliente aprovar) e exporta cada slide em **PNG 1080x1350** pronto para postar. Deriva a paleta inteira de **uma cor só**, escolhe o par de fontes e já embute barra de progresso e seta de swipe dentro da imagem. Mostra o roteiro em texto antes de codar. | Digite `/carrossel` ou peça: *"faz um carrossel sobre X para o cliente Y"*. |
...
páginas/anúncios avulsos, e o `/carrossel` transforma a copy aprovada em post
para o feed.
...
| Carrossel de Instagram (HTML + PNGs) | `clientes/<cliente>/copy/carrosseis/` |
