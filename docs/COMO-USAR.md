...
| **`/conselho-estrategico`** | Comando (decisão) | Monta uma **mesa de 4 conselheiros** (Russell Brunson, Alex Hormozi, Luiz Filho, Matuta) para decidir: oferta, preço, time, operação, caixa, crescimento ou crise. Faz 5 perguntas de diagnóstico, cada conselheiro analisa por uma lente diferente (leitura, riscos, solução, ajuste, critério de sucesso) e a mesa fecha com **síntese**: onde todos concordam, onde discordam e o que fazer esta semana / este mês / neste trimestre. **Não gera copy — gera decisão.** | Digite `/conselho-estrategico`. Na pasta de um cliente, já puxa o contexto dele. |
